...
public class QuestionsTest extends TestFixtures {
    private MainPageObj mainPageObj = new MainPageObj(driver);

    //Заголовок элемента аккордеона "Вопросы о важном"
    private String dropdownHeader;

    //Раскрывающийся текст элемента аккордеона "Вопросы о важном"
    private String dropdownText;

    //Порядковый номер элемента аккордеона "Вопросы о важном"
    private int itemNumber;

    public QuestionsTest(String dropdownHeader, String dropdownText, int itemNumber){
        this.dropdownHeader = dropdownHeader;
        this.dropdownText = dropdownText;
        this.itemNumber = itemNumber;
    }

    @Parameterized.Parameters(name = "CHROME.  Вопрос: {0}, ответ: {1}")
    ...
    public void clickFaqAccordionItemCheckDropdownText(){
        driver.findElement(mainPageObj.getFaqHeader(itemNumber)).click();
        String actualDropdownText = driver.findElement(mainPageObj.getFaqDropdownText(itemNumber)).getText();
        Helpers.waitTextToBePresentInElement(driver,5, mainPageObj.getFaqDropdownText(itemNumber), dropdownText);
